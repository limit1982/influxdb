...
func newEndpointStore(kv Store) *uniqByNameStore {
	...
	return &uniqByNameStore{
		resource:   resource,
		entStore:   newStore(kv, resource, []byte("notificationEndpointv1"), EncIDKey, EncBodyJSON, decEndpointEntFn, decValToEntFn),
		indexStore: newIndexStore(kv, resource, []byte("notificationEndpointIndexv1"), true),
		decodeEntOrgNameFn: func(v interface{}) (influxdb.ID, string, error) {
			edp, ok := v.(influxdb.NotificationEndpoint)
			if err := errUnexpectedDecodeVal(ok); err != nil {
				// should never get here, should be validated several times before here
				return 0, "", err
			}
...
func (s *Service) findNotificationEndpointByName(ctx context.Context, tx Tx, orgID influxdb.ID, name string) (influxdb.NotificationEndpoint, error) {
	...
	body, err := s.endpointStore.FindByName(ctx, tx, Entity{
		OrgID: orgID,
		Name:  name,
	})
	if err != nil {
...
func (s *Service) findNotificationEndpointByID(ctx context.Context, tx Tx, id influxdb.ID) (influxdb.NotificationEndpoint, error) {
	decodedEnt, err := s.endpointStore.FindByID(ctx, tx, id)
	if err != nil {
		return nil, err
	}
...
func (s *Service) findNotificationEndpoints(ctx context.Context, tx Tx, filter influxdb.NotificationEndpointFilter, opt ...influxdb.FindOptions) ([]influxdb.NotificationEndpoint, int, error) {
	...
	err = s.endpointStore.Find(ctx, tx, o, filterEndpointsFn(idMap, filter), func(k []byte, v interface{}) error {
...
func (s *Service) deleteNotificationEndpoint(ctx context.Context, tx Tx, id influxdb.ID) (flds []influxdb.SecretField, orgID influxdb.ID, err error) {
	...
	if err := s.endpointStore.DeleteEnt(ctx, tx, id); err != nil {
		return nil, 0, err
	}
